...
import { ENEMIES } from "./data/terminidEnemies";
...
export default function App() {
  ...
  const [wikiEnemyId, setWikiEnemyId] = useState(ENEMIES[0]!.id);
  const [wikiPartId, setWikiPartId] = useState(ENEMIES[0]!.parts[0]!.id);
  ...
  const wikiPart = useMemo(() => {
    const enemy = ENEMIES.find((e) => e.id === wikiEnemyId);
  ...
  const selectedEnemy = ENEMIES.find((e) => e.id === wikiEnemyId);
